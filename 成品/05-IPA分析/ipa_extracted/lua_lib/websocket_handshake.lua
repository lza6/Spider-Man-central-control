...
local upgrade_request = function(req)
  ...
  local host_header = req.host_header
  if not host_header then
    host_header = req.host
    if host_header and host_header:find(':', 1, true) and host_header:sub(1, 1) ~= '[' then
      host_header = '[' .. host_header .. ']'
    end
    if req.port and req.port ~= 80 then
      host_header = format('%s:%d', host_header, req.port)
    end
  end
  local lines = {
    format('GET %s HTTP/1.1',req.uri or ''),
    format('Host: %s',host_header),
    'Upgrade: websocket',
    'Connection: Upgrade',
