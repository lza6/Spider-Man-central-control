...
local parse_url = function(url)
  ...
  local default_port = DEFAULT_PORTS[protocol]
  local host, port, host_header
  if address:sub(1, 1) == '[' then
    host, port = address:match('^%[([^%]]+)%]:(%d+)$')
    if not host then
      host = address:match('^%[([^%]]+)%]$')
    end
    if not host then error('Invalid URL:'..url) end
  ...
  return protocol, host, port, uri, host_header
end

local resolve_tcp_addresses = function(socket, host)
  local addrinfo, err = socket.dns.getaddrinfo(host)
  if not addrinfo then return nil, err end
  local addresses = {}
  for _, alt in ipairs(addrinfo) do
    if (alt.family == 'inet' or alt.family == 'inet6') and alt.addr then
  ...
  if #addresses == 0 then
    return nil, 'no supported address family'
  end
  return addresses
end

local create_tcp_for_family = function(socket, family)
  if family == 'inet6' then
    return socket.tcp6()
  elseif family == 'inet' then
    return socket.tcp4()
  end
  return nil, 'unsupported address family: ' .. tostring(family)
end
...
return {
  ...
  parse_url = parse_url,
  resolve_tcp_addresses = resolve_tcp_addresses,
  create_tcp_for_family = create_tcp_for_family,
